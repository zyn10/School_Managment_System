...
void Teacher::enter_TeacherData()
{
	...
	while (1)
	{
		...
		cout << "\t\t\t\t\t Enter Teacher CNIC  : ";		cin.ignore();	getline(cin, CNIC);
...
//cout << "Enter Class : ";	cin >> classs;
	...
	switch (opp)
	{
	case '1':          //kuon k opp kchar ha so es ko b comma lagana para or case 2 ko b

	{
		...
		cout << "\t\t\t\t\t --------------------------" << endl;
		cout << "\t\t\t\t\t |                         |" << endl;
		cout << "\t\t\t\t\t |  Enter  : ";              ///////////// yahan endl lagao enter k aagay he lines cout kar dayta ha
		cin >> var[16];
			...
			if (z)
			{
				if (count != 15)
				{
					if (count == 9 || count == 10 || count == 12 || count == 13)
					{
						...
						{
							cout << "\t\t\t\t\t Enter new Salary        : ";		cin >> var[3];
